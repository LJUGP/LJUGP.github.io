...
- 플레이어는 키보드 조작(F1, F2, F3)을 통해 건축, 스탯, 상점 기능을 이용 가능하다.
- 건축, 상점 기능의 이용에는 GOLD가 소모된다.
- 스탯 기능의 이용에는 스탯 포인트가 소모된다.
- 스탯 포인트는 레벨업당 1Point 획득할 수 있다.
- 스탯 포인트로는 연사속도 등의 플레이어 능력치를 상승시킬 수 있다.
- 적을 처치시 경험치를 획득하여 레벨을 올릴 수 있다.
- 게임화면 중앙, 적과 플레이어 사이에는 건축물을 설치 가능한 공간이 있다.
- 플레이어는 건축 기능을 이용해 중앙에 건축물(바리게이트, 포탑)을 설치할 수 있다.
- 건축 기능 이용중에는 설치 모드로 바뀌며 게임이 느려진다.
- 건축물은 정해진 공간에만 설치 가능하다.
- 바리게이트는 적의 접근을 막는다.
- 포탑은 가까운 적을 자동공격한다.
